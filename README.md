...
> **2053 年的洛杉磯，酸雨從不停。你是個被吊銷警徽的偵探 —— 一把槍、半瓶威士忌、一個還沒救回來的女人。**
> Dynamix 1990 年把《銀翼殺手》搬進 14 吋螢幕的賽博龐克黑色電影冒險，三十六年後終於有了繁體中文版：整本劇本 **2,386 句全翻**、桌機到手機**五大平台**，一個 **F8** 在五種字幕語言間切換、一個 **F9** 讓孟波**第一次開口說話**。連標題畫面那行金字，都是引擎即時替他補上的 —— 而且**不動你任何一個遊戲檔**。
...
> 開機選單 →「孟波」在公寓裡的內心獨白，全程繁體中文、真 **24×24 點陣字**，連 UI 都沒放過。

<p align="center">
  <b>全劇本 2,386 句全翻</b>　·　<b>五大平台</b>　·　<b>F8 五種字幕語言</b>　·　<b>F9 四種語音</b>　·　<b>引擎即時金字標題</b>
</p>
...
《Rise of the Dragon》最先教會我的，是「太遲」這兩個字。

它有個在 1990 年很奢侈的設計：遊戲裡的時鐘是**真的在走**。你拖拉、你睡過頭、你晚一步推開那扇門，世界就頭也不回地壞給你看 —— 女友沒救成、線索斷在手裡、偵探死得莫名其妙。可那時候我輸得更冤：不是輸給時間，是輸給看不懂。滿螢幕的英文又黑又冷又好看，我卻只能像隔著一層起霧的玻璃看戲，眼睜睜看主角一腳踩進每一個我沒讀懂的陷阱。

我那時不知道他本名叫 Blade。我擅自在心裡喊他**孟波** —— 反正都是吊兒郎當、神槍在身、把柔軟藏起來的都市獵人，把冴羽獠的名字借給他，剛剛好。我也讀不懂那整場毒品與邪教的陰謀，只好替每個角色配上一套自己亂編的中文台詞，硬把劇情圓回來。那是一個小孩想得到、最笨也最浪漫的玩法。

三十六年後，那座從不等人的城市，總算等到有人替它把字補上。這一次孟波講的每一句 —— 整本 **2,386 句** —— 都是真的中文了；而他另一個身分「城市獵人」，也終於印回了標題畫面。
...
- 預設就是**中文 24×24**。遊戲中按 **F8** 即時循環顯示語言：**中文 24×24 → 中文 16×16 → 德文 → 日文 → 英文**；按 **F9** 獨立循環語音語言（英／日／中／德／關）。
...
故事是這樣的：2053 年，你是 William「Blade」Hunter，一個丟了警徽、靠接私案過活的偵探。文森奇市長的女兒被一種不明物質啃蝕致死，背後牽連到一款叫 **MTZ** 的新型毒品。你被請去查源頭，然後一頭撞進毒梟、幫派與「新黎明教團」的陰謀裡。《電腦遊戲世界》(Computer Gaming World) 在 1996 年把它選進「史上百大遊戲」第 83 名；2000 年的回顧稱它「**被低估的經典，遠遠超前它的時代**」。被低估、超前時代 —— 偏偏它**從頭到尾沒有一個中文版**。對 1990 年代的台灣小孩來說，它就是一款「畫面很猛、英文很硬」、只能半猜半矇玩到底的遊戲。這個專案，就是那道語言之牆遲到三十六年的拆除令：**現在，它有中文版了。**
...
**同一句台詞並排四種顯示模式**（英文 / 中文 24×24 / 中文 16×16 / 德文）：
...
**而現在 F8 多了第五種：日文字幕。** 下一段〈[聲音](#voice)〉會講到，我們用 whisper 把日版的真人配音「聽寫成文字」—— 那份語料沒有只躺在硬碟裡，它直接變成了一個**可玩的顯示模式**。F8 多按一下，酒吧裡那個給線索的角色就整個說起日文：

![日文字幕](screenshots/japanese_subtitle.png)

> 酒保「The Jake」那句線索，跑在 Shift-JIS 點陣字上：「ワラビーストリート544に 奴の隠れがある」（瓦拉比街 544 號，他的藏身處就在那）。日文不像中文走 Big5，得另做一套 Shift-JIS 索引的 24×24 點陣字（[`tools/build_jp_font.py`](tools/build_jp_font.py)）；引擎依字型的 encoding 旗標自動切換查表方式，同一條繪字路徑，中文吃 Big5、日文吃 Shift-JIS。這份字幕是機器聽寫（whisper base 模型）的稿子，用字偶有出入，但每一句都對得上時間軸。
...
講完畫面，來講**聲音** —— 這是 DOS 版玩家當年沒有的東西。

1990 年的 DOS 版是**啞的**：孟波再怎麼嘴賤、阿香再怎麼罵人，全靠你腦補。但兩年後的 **Sega CD 版**砸了當年最奢侈的本錢 —— **真人配音**。美版請了配音員替孟波、傑克、市長獻聲；日版則整套重配成日文。那是 CD-ROM 剛進家用主機、「會說話的遊戲」還能上雜誌封面的年代。

這次我們把**四種語言的聲音**都接回了 DOS 版的對話框，按 **F9** 即時切換：

| 語音 | 來源 | 怎麼來的 |
|---|---|---|
| 🇺🇸 英文 | **US Sega CD 真人原聲** | 從美版光碟資料軌抽出 1352 段 PCM，對回每句對白 |
| 🇯🇵 日文 | **JP Sega CD 真人日配** + TTS 補缺 | 日版抽出 1010 段；日版沒配到的 385 句用日文 TTS 補，聲線比照該角色的真人日配 |
| 🇹🇼 中文 | **神經網路 TTS** | 1307 句、61 個說話人各自分配聲線 |
| 🇩🇪 德文 | **神經網路 TTS** | 官方德文版台詞 + 德文 TTS |

關鍵是:**F9 語音跟 F8 顯示語言完全獨立** —— 你可以**看中文字幕、聽孟波的英文原聲**，也可以看中文、聽日配，怎麼搭都行。

對老玩家來說這裡有個小小的圓滿：那個在台灣被叫了三十年「孟波」、卻一輩子沒在 DOS 版裡開過口的偵探，現在終於有聲音了 —— 你可以聽他的英文原聲，或聽他用日語咬牙。而那行標題上的「城市獵人」金字，現在也對得起聲音了。

> 還有個意外收穫：我們用 whisper 把日版的真人日配**聽寫成文字**，等於繞過了卡關已久的日文自訂編碼逆向 —— 一份 scene-by-scene 的日文台詞語料就這樣有了（見 [Sega CD 日文版](#segacd)）。而這份語料不只能聽：它已經接成 F8 的**第五個顯示模式**，孟波的世界可以整個讀成日文（見 [實機展示](#showcase)）。
...
`VoiceSupport`（`engines/dgds/voice.{h,cpp}`）在每次對話開啟時（`scene.cpp showDialog`），用 `scene:num` 查到該句語音、透過 `Audio::Mixer`（`kSpeechSoundType`）播出。

- **兩套命名**：英/日是真人光碟錄音，照光碟 clip 名（`RDxxxx`），用 `voice.map` 把拆成多段的長台詞 queue 起來接續播；中/德是 TTS，一句一檔照 `scene:num` 命名（`8_4.ogg`）。
- **語音獨立於顯示**：`F9` 切的是 `VoiceSupport` 的 mode，跟 `F8`（CJK 顯示）解耦 —— 四語字幕 × 四語語音任意搭。
- **打包**：四套 wav 共 ~788MB，壓成 **Ogg Vorbis ~99MB（8×）**；`openClip()` 優先讀 `.ogg`、退回 `.wav`。Windows 為此在 mingw、Android 在 NDK sysroot 補編 libogg/libvorbis。
- **日文聽寫**：日版缺配的句子，先用 `faster-whisper` 把真人日配聽寫成日文、機翻校過再餵 TTS；這份聽寫語料同時成了日版文字逆向的旁路（見下章）。

**統計**：對話 2,386 句（100% 覆蓋）、三條繪字路徑全打通、五平台全 ship、真 24×24 Big5 點陣字 + 16×16 副尺寸、F8 循環 4 種**字幕**語言、**F9 循環 4 種語音**（英/日真人 + 中/德 TTS）、城市獵人標題副標 engine overlay。
...
進度誠實說：文字那條路卡關。日文版的字是**自訂編碼**（不是 SJIS、也不是 JIS index），逐字對不上任何標準表；真正的 blocker 是 **script-opcode 逆向** —— 得先搞懂它那套腳本指令怎麼讀字串、怎麼分頁。

但**聲音**這條路意外繞了過去：日版光碟裡有整套**真人日配**，我們做語音功能時把它一段段抽出來，順手用 `faster-whisper` **聽寫成日文文字**。聽寫不是原始官方台詞（專有名詞會聽錯），但已經是一份 scene-by-scene、可對照、可校對的日文語料 —— 卡了很久的「日版有什麼台詞」，先用耳朵問了出來。下一步是拿這份語料回頭對照、慢慢逼近官方原文。（日版實機截圖屬版權素材，未隨本 repo 公開。）

策略是雙軌並進：**機器日文**（先做一版能跑的 —— 現在已經接成 F8 的第五個顯示模式，[實機展示](#showcase)那張日文截圖就是它）與**官方日文**（等 script-opcode 逆向通了再上）分開推進，互不擋路。這一段純屬考古興趣，不影響繁中版的任何功能。
...
開頭那封信說，《Rise of the Dragon》最先教會我的是「太遲」。這一次不一樣了 —— 字補上了、聲音接回來了、連標題那行「城市獵人」金字都印好了。那個三十六年前隔著起霧玻璃看戲的小孩，總算聽得懂孟波在說什麼。
