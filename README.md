...
> 真實 **24×24 點陣中文字**直接畫在遊戲畫面上。按 **F8** 即時循環 5 種顯示模式：
> **英文 / 中文 24×24 / 中文 16×16 / 德文 /（日文，籌備中）**。
...
4. **可以切換語言** — 遊戲中按 **F8** 即時循環 **英文 / 中文 24×24 / 中文 16×16 / 德文**
   （日文籌備中）。原始英文檔完全不動，各語言是「疊」上去的，所以可以共存、一鍵切換。
...
> ✅ **Linux 版已經可以玩了**（實機 headless 驗證渲染中文）。
> 🚧 **Windows 版已交叉編譯完成**（待實機 Windows 驗證）。macOS 有 build recipe（待 Mac 實作）。

1. 準備一份你**合法擁有**的《Rise of the Dragon》遊戲資料夾（內含 `VOLUME.VGA` 等）。
2. 取得對應平台（打包腳本：[`package_linux.sh`](scripts/package_linux.sh)、
   [`package_appimage.sh`](scripts/package_appimage.sh)、[`build_windows.sh`](scripts/build_windows.sh)）：
   - **Linux AppImage**（單檔）：`Rise-of-the-Dragon-CHT-x86_64.AppImage`
   - **Linux tar.gz**：`rotd-cht-linux-x86_64.tar.gz`（解壓後 `./rotd-cht.sh`）
   - **Windows zip**：`rotd-cht-windows-x86_64.zip`（拖放遊戲資料夾到 `play-rotd-cht.bat`）
3. 啟動，指向你的遊戲資料夾：
   ```bash
   # Linux：
   ./Rise-of-the-Dragon-CHT-x86_64.AppImage /路徑/到/你的/遊戲資料夾
   ```
4. 遊戲中按 **F8** 即時循環顯示模式：**英文 / 中文 24×24 / 中文 16×16 / 德文**。

> 中文是「疊」在原版上的 overlay，你手上的原始遊戲檔完全不會被改動。
...
| Phase 2 | 翻譯 overlay + 語言切換鍵（F8） | ✅ 完成 |
| Phase 3 | UI / 按鈕中文化 | ✅ |
| Phase 1.5 | 真 24×24 高解析中文字層 | ✅ |
| Phase 4 | 全量翻譯（2,386 句）+ 排版/翻譯稽核 | ✅ 翻譯+稽核 ・ 🚧 逐場景視覺 QA |
| — | 引擎內建 game-tester（autopilot 自動截圖驗證）| ✅ [報告](docs/GAME_TEST_REPORT.md) |
| Phase 5 | 打包：Linux / Windows / macOS | ✅ Linux ・ ✅ Windows(交叉編譯) ・ 📄 macOS(recipe) |
| Phase 7 | 日文版（Sega CD 官方）逆向 | 🚧 動態 harness 就緒，待 Mega-CD BIOS |
