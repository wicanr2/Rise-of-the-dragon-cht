...
<p align="center">
  <img src="screenshots/jp_choice.png" width="60%" alt="Sega CD 日文版實機畫面"/>
</p>

進度誠實說：卡關。日文版的文字是**自訂編碼**（不是 SJIS、也不是 JIS index），逐字對不上任何標準表；真正的 blocker 是 **script-opcode 逆向** —— 得先搞懂它那套腳本指令怎麼讀字串、怎麼分頁，才有辦法把日文一句句抽出來。動態 harness 已經能在 emulator 裡跑到日文台詞畫面（上圖），但離「乾淨抽出全部日文」還有距離。
