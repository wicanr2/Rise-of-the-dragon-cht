...
> ✅ **Linux 版已經可以玩了。** Windows / macOS 打包進行中。

1. 準備一份你**合法擁有**的《Rise of the Dragon》遊戲資料夾（內含 `VOLUME.VGA` 等）。
2. 取得 Linux 版（擇一，由 [`scripts/package_linux.sh`](scripts/package_linux.sh) /
   [`scripts/package_appimage.sh`](scripts/package_appimage.sh) 產出）：
   - **AppImage**（單檔）：`Rise-of-the-Dragon-CHT-x86_64.AppImage`
   - **tar.gz**（資料夾＋啟動器）：`rotd-cht-linux-x86_64.tar.gz`
3. 啟動，指向你的遊戲資料夾：
   ```bash
   ./Rise-of-the-Dragon-CHT-x86_64.AppImage /路徑/到/你的/遊戲資料夾
   # 或 tar.gz 版：解壓後  ./rotd-cht.sh /路徑/到/遊戲資料夾
   ```
...
| Phase 5 | 打包：Linux（AppImage + tar.gz）/ Windows / macOS | ✅ Linux ・ 🚧 Win/macOS |
