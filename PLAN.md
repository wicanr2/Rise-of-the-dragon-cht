...
### Phase 6 — 多模式顯示切換 ✅（中24/中16/英/德；日文待 Sega CD）
F8 循環 `CJKSupport` 的 `DisplayMode`：英(原始) → 中文24 → 中文16 → 德文。實機驗證 4 模式（`screenshots/showcase/modes.gif`）。德文 = `de.dtr`（CP437，疊在英版上、原字型渲染；少數重音字 ä/ß 因英版字型涵蓋不全略有瑕疵）。日文模式 enum 已預留。
